...
// Tambahkan array warna untuk setiap segmen tubuh ular
const SNAKE_COLORS = ['blue', 'green', 'yellow', 'orange']; // Atau warna yang Anda inginkan

// Modifikasi fungsi draw() untuk menggambar ular dengan warna-warna yang berbeda
function draw() {
    context.clearRect(0, 0, canvas.width, canvas.height);
    drawGrid();

    // Gambar kepala ular
    context.fillStyle = SNAKE_COLORS[0];
    context.fillRect(snake[0].x, snake[0].y, SNAKE_SIZE, SNAKE_SIZE);

    // Gambar segmen-segmen tubuh ular dengan warna yang berbeda
    for (let i = 1; i < snake.length; i++) {
        context.fillStyle = SNAKE_COLORS[i % SNAKE_COLORS.length]; // Gunakan warna dari array dengan modulus untuk mengulang warna
